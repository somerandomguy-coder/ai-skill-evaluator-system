  ...
  return (
    <>
      {highlightSegments(text, quote).map((s, i) =>
        s.mark ? (
          <mark key={i} className="rounded bg-amber-200/70 px-0.5 text-foreground">
            {s.text}
          </mark>
        ) : (
      ...
      {turns.map((t) => {
        ...
        return (
          <li
            ...
            id={`turn-${t.seq}`}
            className={cn(
              "scroll-mt-24 rounded-xl border p-3.5 transition-shadow",
              isUser ? "border-l-4 border-l-indigo-400 bg-card" : "border-l-4 border-l-slate-300 bg-muted/40",
              active && "ring-2 ring-amber-400"
            )}
          >
            <div className="mb-1.5 flex items-center gap-2 text-xs text-muted-foreground">
              {isUser ? <User className="size-3.5" aria-hidden /> : <Bot className="size-3.5" aria-hidden />}
              <span className="font-medium text-foreground/80">Turn {t.seq}</span>
              <span aria-hidden>·</span>
              <span>{isUser ? "Candidate" : "Assistant"}</span>
              <span aria-hidden>·</span>
              <span className="tabular">{offset < 1 ? "start" : `+${formatMinutes(offset)}`}</span>
            </div>
